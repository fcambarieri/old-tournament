...
public class ChoiceField<T> {
    ...
    public ChoiceField(JTextField textField, String title, ChoiceRootNode choiceRootNode, Class selectableClass) {
    ...
    private void textAction() {
        ...
        controller.setTitleForSearch(this.choiceRootNode.getTitleForSearch());
        ...
        if (controller.showModal()) 
            setChoice((T) controller.selectedNodes.get(0));
        else
            setChoice(null);
    }
    ...
    public void setChoice(T newValue) {
        if (writeLock)
            return;
